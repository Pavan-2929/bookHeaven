import React from "react";
import { Link } from "react-router-dom";
import { MdLocationOn } from "react-icons/md";

const SearchListing = ({ listings }) => {
  return (
    <div className="flex justify-around gap-10 flex-wrap">
      {listings.map((listing) => (
        <div
          className="bg-[#282828] shadow-md hover:shadow-lg transition-shadow overflow-hidden rounded-lg w-full sm:w-[300px]"
          key={listing._id}
        >
          <Link to={`/listing/${listing._id}`}>
            <img
              src={listing.imageUrls[0]}
              alt="listing cover"
              className="h-[320px] sm:h-[220px] w-full object-cover hover:scale-105 transition-scale duration-300"
            />
            <div className="p-3 flex flex-col gap-2 w-full">
              <p className="truncate text-lg font-semibold text-purple-700">
                {listing.name}
              </p>
              <div className="flex items-center gap-1">
                <MdLocationOn className="h-4 w-4 text-purple-700" />
                <p className="text-sm text-white truncate w-full">
                  {listing.address}
                </p>
              </div>
              <p className="text-sm text-white line-clamp-1">
                {listing.description}
              </p>
              <p className="mt-2 font-semibold ">
                $
                {listing.offer
                  ? listing.discountPrice.toLocaleString("en-US")
                  : listing.regularPrice.toLocaleString("en-US")}
                {listing.type === "rent" && " / month"}
              </p>
              <div className=" flex gap-4">
                <div className="font-bold text-xs">
                  {listing.bedrooms > 1
                    ? `${listing.bedrooms} beds `
                    : `${listing.bedrooms} bed `}
                </div>
                <div className="font-bold text-xs">
                  {listing.bathrooms > 1
                    ? `${listing.bathrooms} baths `
                    : `${listing.bathrooms} bath `}
                </div>
              </div>
            </div>
          </Link>
        </div>
      ))}
    </div>
  );
};

export default SearchListing;
